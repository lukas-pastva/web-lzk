<?php
  function f($sub_class){
    ...
    <DIV CLASS="stranka">
     ...
     // -------------------------------------------------------------------------- //
     ?>
     <DIV CLASS="cursor" ONCLICK="window.open('admin_pictures_insert.php?x=1', '_blank', 'toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=1,resizable=0,width=<? echo ADMIN_PICTURES_INSERT_W; ?>,height=<? echo ADMIN_PICTURES_INSERT_H; ?>,left=10,titlebar=1');"><B>DOPLNIT OBRAZOK</B></DIV>
     <HR>
       <B>ZOBRAZIT TYP OBRAZKOV:</B>
       <FORM onchange="submit()" ACTION="site.php?x=94" METHOD="post">
        <SELECT NAME="sub_class" SIZE="1">
          <OPTION <? if ($sub_class == "none")              echo "SELECTED"; ?> VALUE="">
         <OPTGROUP LABEL="Hip-Hop">
          <OPTION <? if ($sub_class == "write")             echo "SELECTED"; ?> VALUE="write">Write
          <OPTION <? if ($sub_class == "sketches")          echo "SELECTED"; ?> VALUE="sketches">Scketches
          <OPTION <? if ($sub_class == "stickers")          echo "SELECTED"; ?> VALUE="stickers">Stickers
         </OPTGROUP>
         <OPTGROUP LABEL="Ex-sports">
          <OPTION <? if ($sub_class == "sk8")               echo "SELECTED"; ?> VALUE="sk8">SK8
          <OPTION <? if ($sub_class == "bike")              echo "SELECTED"; ?> VALUE="bike">Bike
          <OPTION <? if ($sub_class == "snb")               echo "SELECTED"; ?> VALUE="snb">SNB
         </OPTGROUP>
         <OPTGROUP LABEL="Parties">
          <OPTION <? if ($sub_class == "hip-hop_fest")      echo "SELECTED"; ?> VALUE="hip-hop_fest">Hip-Hop Fest
          <OPTION <? if ($sub_class == "dasemolina-theend") echo "SELECTED"; ?> VALUE="dasemolina-theend">Da Semolina End
          <OPTION <? if ($sub_class == "dasemolinaxi")      echo "SELECTED"; ?> VALUE="dasemolinaxi">Da Semolinaxi
          <OPTION <? if ($sub_class == "hip-hopsummit1")    echo "SELECTED"; ?> VALUE="hip-hopsummit1">Hip-Hop Summit 1
          <OPTION <? if ($sub_class == "hip-hopsummit3")    echo "SELECTED"; ?> VALUE="hip-hopsummit3">Hip-Hop Summit 3
          <OPTION <? if ($sub_class == "jamza")             echo "SELECTED"; ?> VALUE="jamza">Jam ZA
          <OPTION <? if ($sub_class == "martin")            echo "SELECTED"; ?> VALUE="martin">Martin
          <OPTION <? if ($sub_class == "notakdavaj3")       echo "SELECTED"; ?> VALUE="notakdavaj3">No Tak Davaj 3
          <OPTION <? if ($sub_class == "ca2")               echo "SELECTED"; ?> VALUE="ca2">CA uderground 2
          <OPTION <? if ($sub_class == "ca4")               echo "SELECTED"; ?> VALUE="ca4">CA uderground 4
         </OPTGROUP>
         <OPTGROUP LABEL="Haluze">
          <OPTION <? if ($sub_class == "haluze")            echo "SELECTED"; ?> VALUE="haluze">Haluze
         </OPTGROUP>
         <OPTGROUP LABEL="Kooperacia">
          <OPTION <? if ($sub_class == "xtm")               echo "SELECTED"; ?> VALUE="xtm">XTM crew
         </OPTGROUP>
        </SELECT>
