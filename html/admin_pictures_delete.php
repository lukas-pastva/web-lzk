...
// -------------------------------------------------------------------------- //
  ...
  $hard = strip_tags($hard,'');
    ...
    ?>
      ...
      <SPAN CLASS="cursor" ONCLICK="window.open('admin_pictures_delete.php?x=<?php echo $tabulka; ?>&amp;y=<?php echo $stlpec; ?>&amp;z=<?php echo $hodnota; ?>&amp;hard=1', '', 'toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=1,resizable=0,width=300,height=320,left=10,titlebar=1'); window.close();"><B>ANO</B></SPAN>
      &nbsp;&nbsp;&nbsp;
    ...
    function delete_from_db_hard($tabulka, $stlpec, $hodnota){
      ...
      $src=mysql_query("SELECT * FROM pictures WHERE id = ".$hodnota." ");
      $src=mysql_fetch_array($src);
      $src1=$src['destination'].$src['name'];
      $src2=$src['destination']."small/".$src['name'];
      
      
      if ( ( unlink($src1) ) && ( unlink($src2) ) ){echo "\n<br><br>Subor vymazany"; } else { echo "\n<br><br>Subor sa nepodarilo fyzicky vymazat."; }
